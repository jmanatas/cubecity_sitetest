...
function setupPlayer() {
    ...
    console.log("Avatar initialized - starting to fall");
}
...
function processJumpInput() {
    ...
    console.log("Jump check - Space:", keyStates['Space'], "onFloor:", avatar.onFloor, "isJumping:", avatar.isJumping);
    ...
    if (keyStates['Space'] && avatar.onFloor && !avatar.isJumping) {
        ...
        if (jumpSuccess) {
            ...
            console.log("Jump successful - Space key consumed");
        }
    }
}
...
function controls(deltaTime) {
    ...
    console.log("Animation from controller:", animation, "isJumping:", avatar.isJumping);
}
...
function resetPlayerPosition() {
    ...
    console.log("Player respawned - continuing to fall");
}
    ...
    document.addEventListener('keydown', function(event) {
        ...
        if (event.code === 'KeyB') {
            if (avatar.debugCapsuleMesh) {
                ...
                console.log('Debug capsule:', avatar.debugCapsuleMesh.visible ? 'visible' : 'hidden');
            }
        }
        ...
        if (event.code === 'KeyF') {
            ...
            if (!gravityEnabled && avatar.isJumping) {
                avatar.isJumping = false;
                console.log("Jump interrupted - gravity disabled");
                ...
                const { animation } = avatar.controller.update(0.016, keyStates, avatar.cameraAzimuth);
                avatar.setAnimation(animation);
                console.log("Gravity enabled - returning to:", animation);
            }
            ...
            if (!gravityEnabled) {
                ...
                console.log("Gravity re-enabled - forcing fall with downward velocity");
            }
        }
    });

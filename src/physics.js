...
export class PhysicsWorld {
    ...
    playerCollisions(player) {
        ...
        if (result && result.depth > 0.0001) {
            ...
            if (player.onFloor) {
                console.log("On floor - applying correction");
            ...
            if (player.velocity.y < -0.1) { // Only set false if falling
                ...
                console.log("No collision and falling - not on floor");
            } else {
                console.log("No collision but not falling - keeping previous floor state");
                // Keep the previous onFloor state if we're not actively falling
            }
        }
    }
